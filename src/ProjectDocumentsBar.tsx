...
export function ProjectDocumentsBar({projectId:projectIdProp}:{projectId?:string}){
  ...
  return <>
    ...
    {viewer&&<div className="mobileDrawingViewer"><header><button onClick={()=>setViewer(null)}>‹ Tillbaka</button><div><small>RITNING</small><strong>{viewer.document.title}</strong></div></header><main>{viewerError&&<div className="mobileDrawingMessage">{viewerError}</div>}{!viewerError&&!objectUrl&&<div className="mobileDrawingMessage">Öppnar ritning…</div>}{!viewerError&&objectUrl&&<DrawingAnnotations projectId={projectId} documentId={viewer.document.id} title={viewer.document.title} file={viewer.file} objectUrl={objectUrl} apiBase={API_BASE}/>}</main></div>}
  </>;
}
