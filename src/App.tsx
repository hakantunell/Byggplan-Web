...
export function App(){
  ...
  const loadProjects=useCallback(async()=>{
    if(!navigator.onLine){setApiState('offline');return;}
    try{
      const[projectsResponse]=await Promise.all([fetch(`${API_BASE}/api/projects`),loadMe()]);
      if(!projectsResponse.ok)throw new Error(`API svarade ${projectsResponse.status}`);
  ...
  const apiAction=async(url:string,init?:RequestInit)=>{
    const response=await fetch(`${API_BASE}${url}`,init);
    if(!response.ok){
      const body=await response.json().catch(()=>null) as {error?:string}|null;
      throw new Error(body?.error??'Åtgärden kunde inte genomföras.');
    }
  };
  ...
  const updateActivity=async(taskId:string,activity:Activity)=>{
    ...
    try{
      ...
      setTasks(current=>current.map(task=>task.id!==taskId?task:{...task,status:task.status==='todo'?'active':task.status,activities:task.activities.map(item=>item.id!==activity.id?item:{...item,done:!item.done})}));
    }catch(error){alert(error instanceof Error?error.message:'Aktiviteten kunde inte sparas.');}
  };

  const saveField=async(field:DocumentationField,payload:{valueText?:string|null;valueNumber?:number|null;valueBoolean?:boolean|null})=>{
    try{await apiAction(`/api/documentation-fields/${field.id}`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});await refresh();}
    catch(error){alert(error instanceof Error?error.message:'Dokumentationen kunde inte sparas.');}
  };

  const uploadFieldFile=async(field:DocumentationField,file:File)=>{
    if(!navigator.onLine){alert('Du måste vara online för att ladda upp.');return false;}
    const form=new FormData();form.append('file',file,file.name);
    try{await apiAction(`/api/documentation-fields/${field.id}/files`,{method:'POST',body:form});await refresh();return true;}
    catch(error){alert(error instanceof Error?error.message:'Filen kunde inte laddas upp.');return false;}
  };

  const deleteDocumentationEntry=async(entry:DocumentationEntry)=>{
    if(!confirm(`Ta bort ${entry.originalName??'den här filen'}?`))return;
    try{await apiAction(`/api/documentation-entries/${entry.id}`,{method:'DELETE'});await refresh();}
    catch(error){alert(error instanceof Error?error.message:'Filen kunde inte tas bort.');}
  };
  ...
  const submit=async(task:Task)=>{
    ...
    try{await apiAction(`/api/tasks/${task.id}/review`,{method:'POST'});await refresh();}
    catch(error){alert(error instanceof Error?error.message:'Momentet kunde inte skickas för kontroll.');}
  };

  const approve=async(task:Task)=>{
    try{await apiAction(`/api/tasks/${task.id}/approve`,{method:'POST'});await refresh();}
    catch(error){alert(error instanceof Error?error.message:'Momentet kunde inte godkännas.');}
  };

  const reject=async(task:Task,comment:string)=>{
    try{await apiAction(`/api/tasks/${task.id}/reject`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({comment})});await refresh();}
    catch(error){alert(error instanceof Error?error.message:'Momentet kunde inte avvisas.');throw error;}
  };
  ...
  return <div className="app">
    <header>
      <div><strong>{currentProject.name}</strong><span>ByggPlan{me?` · ${me.displayName}`:''}</span></div>
      <b className={apiState==='connected'?'online':'offline'}>{apiState==='connected'?'● Online':apiState==='offline'?'Offline':'API-fel'}</b>
    </header>
    <main>
      <h1>Arbetsområden</h1>
      <div className="workAreas">
        {grouped.map(area=>{
          ...
          return <section className={`areaCard ${areaOpen?'open':''}`} key={area.id}>
            <button className="areaHeader" onClick={()=>toggle(setOpenAreas,area.id,true)}>
              <span className="areaIcon">{areaIcons[area.name]??'▣'}</span>
              <span className="headerText"><b>{area.name}</b><small>{area.sections.length} arbetsavsnitt · {taskCount} moment</small></span>
              <em>{areaOpen?'−':'+'}</em>
            </button>
            {areaOpen&&<div className="areaBody">
              {area.sections.map(section=>{
                const sectionOpen=openSections.includes(section.id);
                return <section className="sectionCard" key={section.id}>
                  <button className="sectionHeader" onClick={()=>toggle(setOpenSections,section.id)}>
                    <span className="sectionIcon">⌖</span>
                    <span className="headerText"><b>{section.name}</b><small>{section.tasks.length} moment</small></span>
                    <em>{sectionOpen?'−':'+'}</em>
                  </button>
                  {sectionOpen&&<div className="sectionBody">
                    {section.tasks.map(task=>{
                      ...
                      return <article className={`taskCard ${taskOpen?'open':''}`} key={task.id}>
                        <button className="taskHeader" onClick={()=>toggleTask(task)}>
                          <i className={task.status}/>
                          <span className="headerText"><b>{task.title}</b><small>{completed}/{task.activities.length} aktiviteter klara</small></span>
                          {task.status==='review'&&canApprove&&<span className="reviewFlag" title="Väntar på ditt godkännande">!</span>}
                          <span className={`pill ${task.status}`}>{labels[task.status]}</span>
                          <em>{taskOpen?'−':'+'}</em>
                        </button>
                        {taskOpen&&<div className="taskBody">
                          ...
                          <button className="technicalButton technicalFirst" onClick={()=>toggle(setOpenTechnical,task.id)}>
                            <span>📚</span>
                            <span><b>Arbetsunderlag</b><small>{task.technical.length} poster · läs före start</small></span>
                            <em>{openTechnical.includes(task.id)?'−':'+'}</em>
                          </button>
                          {openTechnical.includes(task.id)&&<TechnicalList items={task.technical}/>} 
                          <h3>Aktiviteter</h3>
                          <div className="activityFlow">
                            {task.activities.map((activity,index)=><ActivityRow key={activity.id} activity={activity} index={index} task={task} technical={task.technical} expanded={openActivityId===activity.id} isNext={activity.id===nextActivityId} onToggle={()=>setOpenActivityId(current=>current===activity.id?null:activity.id)} onUpdate={updateActivity} onSaveField={saveField} onUpload={uploadFieldFile} onDeleteEntry={deleteDocumentationEntry}/>)}
                          </div>
                          {(task.status==='todo'||task.status==='active')&&<button className="submitButton fullWidth" onClick={()=>void submit(task)}>
                            <span>➤</span><span><b>Skicka för kontroll</b><small>När allt är klart</small></span>
                          </button>}
...
function ReviewNotice({supervisor}:{supervisor:boolean}){
  return <div className="reviewNotice">
    <span>⏳</span>
    <div><b>{supervisor?'Momentet väntar på din kontroll':'Väntar på arbetsledarens kontroll'}</b><small>{supervisor?'Granska aktiviteter och dokumentation innan du godkänner eller avvisar.':'Momentet kan inte ändras till klart förrän arbetsledaren har granskat det.'}</small></div>
  </div>;
}
...
function ReviewActions({task,onApprove,onReject}:{task:Task;onApprove:(task:Task)=>Promise<void>;onReject:(task:Task,comment:string)=>Promise<void>}){
  ...
  return <div className="reviewActions">
    ...
    {!rejecting?<div className="reviewButtons">
      ...
      <div>
        <button type="button" className="cancelReject" disabled={busy} onClick={()=>{setRejecting(false);setComment('');}}>Avbryt</button>
        <button type="button" className="confirmReject" disabled={busy||!comment.trim()} onClick={async()=>{setBusy(true);try{await onReject(task,comment);setComment('');setRejecting(false);}finally{setBusy(false);}}}>Avvisa moment</button>
      </div>
    </div>}
  </div>;
}
...
function ActivityDetailSupport({item}:{item:TechnicalItem}){return <details className="detailSupport"><summary><span>🔎</span><span><b>Detaljunderlag</b><small>{item.title}</small></span><em>+</em></summary><div className="detailSupportBody">{item.summary&&<p>{item.summary}</p>}{item.revision&&<small>Revision: {item.revision}</small>}{item.details?.length?<ul>{item.details.map(detail=><li key={detail}>{detail}</li>)}</ul>:null}{(item.externalUrl||item.objectKey)&&<button type="button">Öppna underlaget</button>}</div></details>;}
...
function DocumentationInput({field,onSave,onUpload,onDeleteEntry}:{field:DocumentationField;onSave:(field:DocumentationField,payload:{valueText?:string|null;valueNumber?:number|null;valueBoolean?:boolean|null})=>Promise<void>;onUpload:(field:DocumentationField,file:File)=>Promise<boolean>;onDeleteEntry:(entry:DocumentationEntry)=>Promise<void>}){
  const[uploading,setUploading]=useState(false);const entry=field.entries[0];
  if(field.type==='photo'||field.type==='file')return <div className="docField fileField"><div><b>{field.label}</b>{field.helpText&&<small>{field.helpText}</small>}</div><label className={`filePicker ${uploading?'busy':''}`}><input type="file" accept={field.type==='photo'?'image/*':undefined} capture={field.type==='photo'?'environment':undefined} disabled={uploading||Boolean(field.maximumItems&&field.entries.length>=field.maximumItems)} onChange={async event=>{const file=event.target.files?.[0];event.target.value='';if(!file)return;setUploading(true);try{await onUpload(field,file);}finally{setUploading(false);}}/><span>{uploading?'Laddar upp…':field.type==='photo'?'📷 Ta foto / välj bild':'📎 Lägg till fil'}</span><strong>{field.entries.length}{field.minimumItems?`/${field.minimumItems}`:''}</strong></label>{field.entries.length>0&&<div className="fileEntries">{field.entries.map(item=><div key={item.id}><span>{item.contentType?.startsWith('image/')?'🖼':'📎'}</span><b>{item.originalName??'Fil'}</b><button type="button" onClick={()=>void onDeleteEntry(item)} aria-label="Ta bort">×</button></div>)}</div>}</div>;
