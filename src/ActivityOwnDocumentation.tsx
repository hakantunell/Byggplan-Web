...
export function ActivityOwnDocumentation({activityId,activityType,readOnly}:{activityId:string;activityType:ActivityType;readOnly:boolean}){
  ...
  useEffect(()=>{setOpen(activityType==='measurement');setLoaded(false);setNote('');setFiles([]);setMessage('')},[activityId,activityType]);
  ...
  async function upload(selected:File[]){
    if(!selected.length)return;setBusy(true);setMessage(`Laddar upp ${selected.length} bild${selected.length===1?'':'er'}…`);
    let failed=0;
    ...
    setMessage(failed?`${selected.length-failed} bilder uppladdade, ${failed} misslyckades.`:'');
  }
  ...
  async function openFile(file:OwnFile){
    ...
    }catch{setMessage('Kunde inte öppna bilden.')}
  }

  const label=activityType==='measurement'?'Registrering och egna foton':'Egen dokumentation';
  ...
  return <section className={`ownDocumentation ${open?'open':''}`}>
    ...
    {open&&<div className="ownDocumentationBody">{!loaded?<p>Laddar…</p>:<>
      ...
      <div className="ownPhotos"><div><b>Foton</b><small>Du kan välja flera bilder samtidigt.</small></div>{!readOnly&&<label className={`ownPhotoPicker ${busy?'busy':''}`}><input type="file" accept="image/*" multiple disabled={busy} onChange={event=>{const selected=Array.from(event.target.files||[]);event.target.value='';if(selected.length)void upload(selected)}}/><span>{busy?'Laddar upp…':'📷 Ta foto / välj bilder'}</span></label>}
      {files.length>0&&<div className="ownPhotoList">{files.map(file=><div key={file.id}><button type="button" className="ownPhotoOpen" onClick={()=>void openFile(file)}><span>🖼</span><span><b>{file.originalName}</b><small>{formatBytes(file.sizeBytes)}</small></span></button>{!readOnly&&<button type="button" className="ownPhotoDelete" onClick={()=>void remove(file)} aria-label="Ta bort">×</button>}</div>)}</div>}</div>
