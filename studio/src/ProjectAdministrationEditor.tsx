...
type Item={id:string;code:string;title:string;completed:number;value_text:string;note:string;sort_order:number;data?:Record<string,string>;source_activity_id?:string|null;source_path?:string};
...
export function ProjectAdministrationEditor({projectId}:{projectId:string}){
  ...
  return <section className="projectAdministration"><div className="projectAdministrationHeader"><div><small>ADMINISTRATION</small><h2>Administrativa kontrollpunkter</h2><p>Aktiva Studio-/administrationsaktiviteter från projektstrukturen visas automatiskt här tillsammans med projektspecifika administrativa punkter.</p></div><button className="primary" onClick={newItem}>＋ Ny punkt</button></div><div className="projectAdministrationProgress"><b>{done}/{items.length}</b><span>klara</span></div>{message&&<div className="projectSupportMessage">{message}</div>}{loading&&<div className="projectSupportEmpty">Laddar administrativa punkter…</div>}{!loading&&editingId==='new'&&<div className="projectSupportForm"><label><span>Rubrik</span><input autoFocus value={title} onChange={e=>setTitle(e.target.value)} placeholder="Ex. Försäkringsbevis inskickat"/></label><label><span>Uppgift / värde</span><input value={valueText} onChange={e=>setValueText(e.target.value)} placeholder="Namn, datum eller referens"/></label><label><span>Kommentar</span><textarea rows={4} value={note} onChange={e=>setNote(e.target.value)} placeholder="Valfri kommentar…"/></label><div><button onClick={cancel}>Avbryt</button><button className="primary" disabled={busy||!title.trim()} onClick={()=>void saveNew()}>{busy?'Sparar…':'Spara'}</button></div></div>}{!loading&&editingId!=='new'&&<div className="projectAdministrationList">{items.map(item=>{
    ...
    return <article className={item.completed?'done':''} key={item.id}><label className="adminCheck"><input type="checkbox" checked={Boolean(item.completed)} disabled={busy} onChange={e=>void update(item,{completed:e.target.checked})}/><span><b>{item.title}</b><small>{item.completed?'Verifierad':'Ej verifierad'}</small>{item.source_activity_id&&item.source_path&&<small>Från projektstruktur: {item.source_path}</small>}</span></label>
      ...
      <label><span>Kommentar</span><textarea rows={2} defaultValue={item.note||''} placeholder="Valfri kommentar…" onBlur={e=>{if(e.target.value!==item.note)void update(item,{note:e.target.value})}}/></label>{item.source_activity_id?<small>Rubrik, placering och klassificering ändras på aktiviteten i projektstrukturen.</small>:<button className="danger adminDelete" onClick={()=>void remove(item)}>Ta bort</button>}</article>
  })}</div>}</section>
}
