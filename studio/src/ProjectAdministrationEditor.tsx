...
export function ProjectAdministrationEditor({projectId}:{projectId:string}){
  ...
  async function remove(item:Item){if(item.source_activity_id)return;if(!confirm(`Ta bort ”${item.title}”?`))return;const r=await fetch(`/api/studio/project-administration/${encodeURIComponent(item.id)}`,{method:'DELETE'});if(r.ok)await load()}
