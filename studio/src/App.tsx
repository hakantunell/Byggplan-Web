...
export function App() {
  ...
  return <div className="studio">
    ...
    <aside className="rail">
      <button className="active" title="Projekt">🌳<span>Projekt</span></button>
      <button disabled title="Kontrollplan – import byggs i nästa steg">📋<span>Kontrollplan</span></button>
...
function Inspector({ selection, value, structure, project, onSave, onDelete }: { selection: Selection | null; value: any; structure: Structure; project?: Project; onSave: (form: { name: string; description: string; activityType: string }) => Promise<void>; onDelete: () => Promise<void> }) {
  ...
  useEffect(() => {
    ...
    setActivityType(toEditorActivityType(value?.activity_type));
    setSaving(false);
  ...
  return <section className="inspectorCard">
    ...
    {selection.kind === 'activity' && <label><span>Aktivitetstyp</span><select value={activityType} onChange={event => setActivityType(event.target.value)}><option value="work">Utför</option><option value="documentation">Dokumentera</option><option value="measurement">Mät</option><option value="control">Kontrollera</option><option value="approval">Godkänn</option><option value="wait">Anteckning / vänta</option></select></label>}
    {selection.kind === 'task' && <div className="propertyRow"><span>Status</span><b>{value.status}</b></div>}
    <div className="propertyRow"><span>{selection.kind === 'activity' ? 'Dokumentationsfält' : 'Underliggande objekt'}</span><b>{childCount || 0}</b></div>
    {selection.kind === 'activity' && <ClassificationEditor activityId={selection.id} />}
...
function toEditorActivityType(value: string | undefined) {
  return ({ perform: 'work', document: 'documentation', measurement: 'measurement', check: 'control', approval: 'approval', note: 'wait', choice: 'wait' } as Record<string, string>)[value || ''] || value || 'work';
}
