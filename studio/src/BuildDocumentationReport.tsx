...
export function BuildDocumentationReport({projectId,projectName}:{projectId:string;projectName:string}){
 const[evidence,setEvidence]=useState<Evidence[]>([]),[projectInfo,setProjectInfo]=useState<ProjectInformation|null>(null),[layout,setLayout]=useState<Layout>(()=>readLayout(projectId)),[selectedSection,setSelectedSection]=useState(''),[loading,setLoading]=useState(true),[error,setError]=useState('');
 useEffect(()=>{setLayout(readLayout(projectId));setSelectedSection('');setLoading(true);setError('');Promise.all([
 ...
 ]).then(([items,info])=>{setEvidence(items);setProjectInfo(info)}).catch(e=>setError(e instanceof Error?e.message:'Kunde inte läsa byggdokumentationen.')).finally(()=>setLoading(false))},[projectId]);
 ...
 function selectAndScroll(id:string){setSelectedSection(id);window.requestAnimationFrame(()=>{const key=id||'__cover__';document.querySelector<HTMLElement>(`.buildDocPreview [data-report-section="${CSS.escape(key)}"]`)?.scrollIntoView({behavior:'smooth',block:'start'})})}
