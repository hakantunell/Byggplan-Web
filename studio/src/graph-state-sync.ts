...
async function putState(projectId:string){
  ...
  try{await fetch(`/api/studio/projects/${encodeURIComponent(projectId)}/graph-state`,{method:'PUT',headers:{'content-type':'application/json'},body:JSON.stringify(state)})}catch{}
}

function schedulePut(projectId:string){
  if(!projectId)return;
  const old=timers.get(projectId);if(old)window.clearTimeout(old);
  timers.set(projectId,window.setTimeout(()=>{timers.delete(projectId);void putState(projectId)},300));
}
...
async function loadOrMigrate(projectId:string){
  ...
  try{
    ...
    if(data?.exists){
      ...
      if(hasState(local))await fetch(`/api/studio/projects/${encodeURIComponent(projectId)}/graph-state`,{method:'PUT',headers:{'content-type':'application/json'},body:JSON.stringify(local)});
    }
  }catch{}finally{
