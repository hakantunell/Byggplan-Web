...
export function GraphicalPlanView({projectId,projectName}:Props){
 ...
 const positions=useMemo(()=>{
  ...
  const xGap=250,yGap=112,nodeW=190,maxRows=Math.max(...stages.map(s=>s.length),1);
  for(let stage=0;stage<stages.length;stage++){
   const nodes=stages[stage];
   const offset=Math.max(0,(maxRows-nodes.length)*yGap/2);
   nodes.forEach((node,index)=>map.set(node.task.id,{x:38+stage*xGap,y:55+offset+index*yGap}));
  }
  return{map,width:Math.max(700,80+stages.length*xGap+nodeW),height:Math.max(480,120+maxRows*yGap)};
 },[stages]);
