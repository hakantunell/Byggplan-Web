...
function syncActive(){
 ...
 for(const button of Array.from(document.querySelectorAll<HTMLButtonElement>('.rail .projectUtilityRailButton')))button.classList.toggle('active',button.dataset.utilityLabel===activeLabel);
 const workspace=document.querySelector<HTMLElement>('.projectWorkspace');workspace?.classList.toggle('projectUtilityView',utilityActive);
}
function openUtility(label:UtilityLabel,attempt=0){hideNativeUtilityRows();const row=findRow(label);if(row){row.click();window.setTimeout(syncActive,30);return}const projectButton=document.querySelector<HTMLButtonElement>('.rail button[title="Projekt"]');if(projectButton&&!document.querySelector('.projectWorkspace'))projectButton.click();if(attempt<30)window.setTimeout(()=>openUtility(label,attempt+1),50)}
