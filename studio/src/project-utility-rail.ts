...
function syncActive(){const activeLabel=projectRows().find(row=>row.classList.contains('selected'))?.querySelector('.projectTreeLabel')?.textContent?.trim()||'';const nativeControlPlan=Boolean(document.querySelector('.studioShell.view-native-control-plan'));const graphicalPlan=Boolean(document.querySelector('.studioShell.view-graphical-plan'));const utilityActive=utilityItems.some(item=>item.label===activeLabel);for(const button of Array.from(document.querySelectorAll<HTMLButtonElement>('.rail .projectUtilityRailButton')))button.classList.toggle('active',button.dataset.utilityLabel===activeLabel||(nativeControlPlan&&button.dataset.utilityLabel==='Kontrollplan')||(graphicalPlan&&button.dataset.utilityLabel==='Grafisk plan'));const workspace=document.querySelector<HTMLElement>('.projectWorkspace');workspace?.classList.toggle('projectUtilityView',utilityActive)}
