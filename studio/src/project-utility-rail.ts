...
function hideNativeUtilityRows(){
  for(const row of projectRows())if(utilityItems.some(item=>item.label===rowLabel(row)))row.style.display='none';
}
...
function openUtility(label:UtilityLabel,attempt=0){
  ...
  if(row){row.click();window.setTimeout(syncActive,30);return;}
  const projectButton=document.querySelector<HTMLButtonElement>('.rail button[title="Projekt"]');
  if(projectButton&&!document.querySelector('.projectWorkspace'))projectButton.click();
