...
export function StudioShell(){
 const[view,setView]=useState<StudioView>('project');const[projects,setProjects]=useState<Project[]>([]);const[projectId,setProjectId]=useState('');
 ...
 if(view==='project')return <div className="studioShell view-project">{projectId?<ProjectWorkspace projects={projects} projectId={projectId} onProjectChange={selectProject} onOpenGoverningDocuments={()=>setView('governing-documents')} onOpenMasterProjects={()=>setView('master-projects')}/>:<div className="workspaceEmpty">Hämtar projekt…</div>}</div>;
