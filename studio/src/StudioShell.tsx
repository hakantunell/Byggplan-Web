...
export function StudioShell(){
 ...
 useEffect(()=>{const open=()=>setView('governing-mapping');window.addEventListener('byggplan:open-governing-mapping',open);return()=>window.removeEventListener('byggplan:open-governing-mapping',open)},[]);
 useEffect(()=>{if(projectId)void loadMappingWarning()},[projectId,view,statusVersion]);
 useEffect(()=>{window.dispatchEvent(new CustomEvent('byggplan:mapping-warning',{detail:{count:mappingWarning}}))},[mappingWarning,view]);
