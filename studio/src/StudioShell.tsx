...
import { GoverningMappingRailBridge } from './GoverningMappingRailBridge';
...
export function StudioShell(){
 ...
 if(view==='project')return <div className="studioShell view-project">{projectId?<><ProjectWorkspace projects={projects} projectId={projectId} onProjectChange={selectProject} onOpenGoverningDocuments={()=>setView('governing-documents')} onOpenMasterProjects={()=>setView('master-projects')}/><GoverningMappingRailBridge warningCount={mappingWarning} onOpen={()=>setView('governing-mapping')}/><ProjectHierarchyIndicators key={`indicators-${statusVersion}`}/><ProjectHierarchyStatusV2 key={`status-${statusVersion}`}/><GoverningCompletionIndicator/><ProjectReportsMount projectId={projectId}/><ProjectControlPlanMount projectId={projectId}/></>:<div className="workspaceEmpty">Hämtar projekt…</div>}</div>;
