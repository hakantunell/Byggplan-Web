...
type StudioView='project'|'governing-documents'|'governing-mapping'|'master-projects'|'users';
...
export function StudioShell(){
 ...
 useEffect(()=>{const openUsers=()=>setView('users');window.addEventListener('byggplan:open-users',openUsers);return()=>window.removeEventListener('byggplan:open-users',openUsers)},[]);
 ...
 const current=projects.find(p=>p.id===projectId);const master=view==='master-projects',mapping=view==='governing-mapping',users=view==='users';
 return <div className={`studioShell view-control-plan ${master?'view-master-projects':mapping?'view-governing-mapping':users?'view-users':'view-governing-documents'}`}><div className="controlPlanStudioFrame"><header className="topbar controlPlanTopbar"><div className="brand"><span>BP</span><div><strong>ByggPlan Studio</strong><small>{master?'Masterprojekt':mapping?'Kartläggning':users?'Användare':'Styrdokument'}</small></div></div>{!master&&<select value={projectId} onChange={e=>selectProject(e.target.value)}>{projects.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select>}{master&&<div className="masterTopbarLabel">Bibliotek</div>}<div className="connection ready">● Ansluten</div></header><aside className="rail controlPlanRail"><button title="Projekt" onClick={()=>setView('project')}>🌳<span>Projekt</span></button><button className={view==='governing-documents'?'active':''} title="Styrdokument" onClick={()=>setView('governing-documents')}>📚<span>Styrdokument</span></button><button className={`governingMappingRailButton ${mapping?'active':''}`} title={mappingWarning>0?`Kartläggning – ${mappingWarning} punkt${mappingWarning===1?'':'er'} behöver åtgärdas`:'Kartläggning'} onClick={()=>setView('governing-mapping')}><span className="railIconWrap">🧭{mappingWarning>0&&<em className="railWarningBadge">⚠</em>}</span><span>Kartläggning</span></button><button className={master?'active':''} title="Masterprojekt" onClick={()=>setView('master-projects')}>🏠<span>Masterprojekt</span></button><button className={users?'active':''} title="Användare" onClick={()=>setView('users')}>👥<span>Användare</span></button></aside><section className="controlPlanMainRegion" aria-label={master?'Masterprojekt':mapping?`Kartläggning för ${current?.name||'projektet'}`:users?`Användare i ${current?.name||'projektet'}`:`Styrdokument för ${current?.name||'projektet'}`}>{master?<MasterProjectsView onProjectCreated={openCreatedProject}/>:projectId?(users?<ProjectUsersView projectId={projectId}/>:mapping?<GoverningMappingView projectId={projectId}/>:<GoverningDocumentsWorkspace projectId={projectId} onOpenMapping={()=>setView('governing-mapping')}/>):<div className="empty"><span>{users?'👥':mapping?'🧭':'📚'}</span><h2>Inget projekt valt</h2></div>}</section></div></div>
}
