...
function MasterSnapshotInfo({projectId}:{projectId:string}){
  ...
  async function repair(){setRepairing(true);setMessage('Reparerar Master v2…');try{const mr=await fetch('/api/studio/master-projects',{cache:'no-store'});const md=await responseData(mr) as {error?:string};if(!mr.ok)throw new Error(md.error||`Masterprojektet kunde inte reconcileras (HTTP ${mr.status}).`);setMessage('Synkar projektet och städar ersatta aktiviteter…');const r=await fetch(`/api/studio/projects/${encodeURIComponent(projectId)}/repair-from-master`,{method:'POST'});const d=await responseData(r) as {created?:{areas:number;sections:number;tasks:number;activities:number;retired?:number};error?:string;stage?:string};if(!r.ok)throw new Error(d.error||`Projektet kunde inte repareras (HTTP ${r.status}${d.stage?` · ${d.stage}`:''}).`);await load();const c=d.created;setMessage(`Klart: ${c?.activities||0} nya aktiviteter lades till och ${c?.retired||0} ersatta aktiviteter pensionerades.`);window.setTimeout(()=>window.location.reload(),900)}catch(e){setMessage(e instanceof Error?e.message:'Projektet kunde inte repareras.')}finally{setRepairing(false)}}
  if(!data)return null;
  ...
  return <section className="projectSupportCard"><small>MASTER-SNAPSHOT</small><strong>{data.masterProjectName||data.masterProjectCode||'Masterprojekt'} · v{data.snapshotVersion||'?'}</strong><p>Kod: {data.masterProjectCode||'—'} · Nuvarande Master: v{data.currentMasterVersion||'?'} · {data.activityCount||0} aktiva projektaktiviteter · {data.linkedActivityCount||0} aktiva Master-kopplade.</p><p><b>Valda moduler:</b> {(data.selectedModuleCodes||[]).join(', ')||'inga'}</p>{check&&<p><b>Diagnostik:</b> {check.presentCount}/{check.total} kontrollaktiviteter som ska finnas i v9–v11 hittades.</p>}{newerMaster&&<p><b>Nyare Master finns:</b> projektets snapshot är v{snapshotVersion}, aktuell Master är v{currentMasterVersion}. Reparera projektet för att synka nya aktiviteter och pensionera ersatta dubbletter utan att påverka styrdokumenten.</p>}{missing.length>0&&<details><summary>{missing.length} förväntade aktiviteter saknas</summary><ul>{missing.map(title=><li key={title}>{title}</li>)}</ul></details>}{needsRepair&&<button className="primary" disabled={repairing} onClick={()=>void repair()}>{repairing?'Reparerar…':'Reparera projekt från Master'}</button>}{message&&<p>{message}</p>}</section>;
}
