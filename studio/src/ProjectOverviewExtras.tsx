...
function MasterSnapshotInfo({projectId}:{projectId:string}){
  ...
  async function repair(){setRepairing(true);setMessage('Reparerar Master v2…');try{const mr=await fetch('/api/studio/master-projects',{cache:'no-store'});const md=await responseData(mr) as {error?:string};if(!mr.ok)throw new Error(md.error||`Masterprojektet kunde inte reconcileras (HTTP ${mr.status}).`);setMessage('Fyller på saknade projektaktiviteter…');const r=await fetch(`/api/studio/projects/${encodeURIComponent(projectId)}/repair-from-master`,{method:'POST'});const d=await responseData(r) as {created?:{areas:number;sections:number;tasks:number;activities:number};error?:string;stage?:string};if(!r.ok)throw new Error(d.error||`Projektet kunde inte repareras (HTTP ${r.status}${d.stage?` · ${d.stage}`:''}).`);await load();const c=d.created;setMessage(`Klart: ${c?.activities||0} aktiviteter, ${c?.tasks||0} moment och ${c?.sections||0} avsnitt lades till.`);window.setTimeout(()=>window.location.reload(),900)}catch(e){setMessage(e instanceof Error?e.message:'Projektet kunde inte repareras.')}finally{setRepairing(false)}}
  if(!data)return null;
