...
export function ReportApprovalAction({projectId,itemId,responsibleRole,canAttest,onDone}:{projectId:string;itemId:string;responsibleRole?:string;canAttest:boolean;onDone?:()=>void}){
 ...
 return <div className={`reportApprovalAction ${canAttest?'ready':'locked'}`}><select value={role} disabled={!canAttest||busy} onChange={e=>setRole(e.target.value)}>{roles.map(r=><option key={r}>{r}</option>)}</select><button disabled={!canAttest||busy} onClick={()=>void approve()}>{busy?'Attesterar…':'✓ Attestera'}</button>{!canAttest&&<small>Kan attesteras när punkten är klar.</small>}{error&&<small>{error}</small>}</div>
}
