...
const REFINEMENT_PREFIX='byggplan.graph.refinement.v24.';
...
function refineDependencies(projectId:string,tasks:TaskLike[]){
  ...
  const water=find(tasks,/anslut gemensamt vatten/);
  ...
  add(finalDocs,[useSafety].filter(Boolean));
...
export function installGraphPlanStructureRefinement(){
  const marker='__byggplanGraphPlanStructureRefinementV24';const w=window as typeof window&Record<string,unknown>;if(w[marker])return;w[marker]=true;
