import React from 'react';
import ReactDOM from 'react-dom/client';
import './same-origin-api';
import './mapping-dialog-clarity';
import './graph-wheel-guard';
import { installExecutionContextBridge } from './execution-context-bridge';
import { installProjectUtilityRail } from './project-utility-rail';
import { installGraphicalPlanPdfExport } from './graphical-plan-pdf';
import { installGraphPlanStructureRefinement } from './graph-plan-structure-refinement';
import { installGraphActivityOverlay } from './graph-activity-overlay';
import { installGraphActivityDeduplicate } from './graph-activity-deduplicate';
import { installGraphNodeIconPreserver } from './graph-node-icon-preserver';
import { installActivityMoveBridge } from './activity-move-bridge';
import { installMasterRepairBridge } from './master-repair-bridge';
import { installTaskDescriptionRefreshBridge } from './task-description-refresh-bridge';
import { installGraphDependencyOrphanRepair } from './graph-dependency-orphan-repair';
import { installGoverningReportActivityDetails } from './governing-report-activity-details';
import { installGoverningReportOutcomeOnly } from './governing-report-outcome-only';
import { installHierarchyEditGuard } from './hierarchy-edit-guard';
import { installDrawingPaletteBridge } from './drawing-palette-bridge';
import { installDrawingPaletteWorkflowUx } from './drawing-palette-workflow-ux';
import { installDrawingWorkspaceUx } from './drawing-workspace-ux';
import { installDrawingPaletteSync } from './drawing-palette-sync';
import { StudioShell } from './StudioShell';
import { MasterV2UpgradeAction } from './MasterV2UpgradeAction';
import { ProjectInformationMount } from './ProjectInformationMount';
import { SystemBackupSettingsMount } from './SystemBackupSettingsMount';
import { ProjectExecutionResetSettingsMount } from './ProjectExecutionResetSettingsMount';
import { AuthGate } from './AuthGate';
import './styles.css';
import './palette.css';
import './classifications.css';
import './control-plan.css';
import './governing-documents.css';
import './governing-verifications.css';
import './governing-verification-lock.css';
import './governing-mapping.css';
import './governing-mapping-exception.css';
import './mapping-dialog-clarity.css';
import './studio-shell.css';
import './master-projects.css';
import './project-support.css';
import './project-documents.css';
import './project-documents-workspace.css';
import './drawing-palette-bridge.css';
import './drawing-palette-workflow-ux.css';
import './drawing-workspace-ux.css';
import './project-administration.css';
import './project-detail-tabs.css';
import './project-activities.css';
import './activity-move.css';
import './project-workspace-v2.css';
import './projects-view.css';
import './api-browser.css';
import './governing-import-action.css';
import './governing-analysis-action.css';
import './governing-version-action.css';
import './governing-workspace-nav.css';
import './governing-mapping-rail.css';
import './project-utility-rail.css';
import './graphical-plan.css';
import './graph-activity-overlay.css';
import './governing-report-activity-details.css';

installExecutionContextBridge();
installProjectUtilityRail();
installGraphicalPlanPdfExport();
installGraphPlanStructureRefinement();
installGraphActivityOverlay();
installGraphActivityDeduplicate();
installGraphNodeIconPreserver();
installActivityMoveBridge();
installMasterRepairBridge();
installTaskDescriptionRefreshBridge();
installGraphDependencyOrphanRepair();
installGoverningReportActivityDetails();
installGoverningReportOutcomeOnly();
installHierarchyEditGuard();
installDrawingPaletteSync();
installDrawingPaletteBridge();
installDrawingPaletteWorkflowUx();
installDrawingWorkspaceUx();
ReactDOM.createRoot(document.getElementById('root')!).render(<React.StrictMode><AuthGate><StudioShell/><ProjectInformationMount/><SystemBackupSettingsMount/><ProjectExecutionResetSettingsMount/><MasterV2UpgradeAction/></AuthGate></React.StrictMode>);