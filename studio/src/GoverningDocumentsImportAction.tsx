...
export function GoverningDocumentsImportAction(){
  const[target,setTarget]=useState<Element|null>(null);const[projectId,setProjectId]=useState('');const[open,setOpen]=useState(false);const[busy,setBusy]=useState(false);const[message,setMessage]=useState('');
  ...
  useEffect(()=>{const sync=()=>{const nextTarget=document.querySelector('.governingPrimaryView .governingListHeader');const select=document.querySelector('.studio .topbar select') as HTMLSelectElement|null;setTarget(current=>current===nextTarget?current:nextTarget);setProjectId(select?.value||'')};sync();const timer=window.setInterval(sync,250);return()=>window.clearInterval(timer)},[]);
