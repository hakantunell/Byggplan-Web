...
const REPAIR_PREFIX='byggplan.graph.orphan-repair.v1.';
...
export function installGraphDependencyOrphanRepair(){
 const run=async()=>{
  ...
  try{
   ...
   const children=new Map<string,number>();for(const parents of Object.values(valid))for(const parent of parents)children.set(parent,(children.get(parent)||0)+1);
   const suggested=suggestions(structure);const roots=new Set(structure.tasks.filter(t=>/förbered byggstart|starta byggarbetsplats|startbesked|byggstart/i.test(text(t,structure))).map(t=>t.id));
   let repaired=0;
   for(const task of order(structure.tasks)){
    const id=task.id;if(roots.has(id))continue;
    const existedInStored=Object.prototype.hasOwnProperty.call(stored,id);const hasValidParent=(valid[id]||[]).length>0;const hasValidChild=(children.get(id)||0)>0;
    const staleOwnParents=(stored[id]||[]).some(parent=>!ids.has(parent));
    const newOrOrphan=!existedInStored||staleOwnParents||(!hasValidParent&&!hasValidChild);
    if(!newOrOrphan||hasValidParent)continue;
    const parents=unique((suggested[id]||[]).filter(parent=>ids.has(parent)&&parent!==id));if(!parents.length)continue;
    valid[id]=parents;for(const parent of parents)children.set(parent,(children.get(parent)||0)+1);repaired++;
   }
