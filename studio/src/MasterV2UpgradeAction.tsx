...
export function MasterV2UpgradeAction(){const[target,setTarget]=useState<Element|null>(null);const[masters,setMasters]=useState<Master[]>([]);const[selectedName,setSelectedName]=useState('');const[busy,setBusy]=useState(false);const[message,setMessage]=useState('');
 ...
 const upgradePortal=target&&selected&&selected.code==='fritidshus-v2'&&Number(selected.version)<TARGET_VERSION?createPortal(<div style={{display:'flex',alignItems:'center',gap:8}}><button onClick={()=>void upgrade()} disabled={busy}>↻ Uppgradera till version {TARGET_VERSION}</button>{message&&<small>{message}</small>}</div>,target):null;
 return <><ProjectHierarchyIndicators/>{upgradePortal}</>}
